...
const OICF_Maps = (() => {
  ...
  function renderWorldMarkers(layer) {
    ...
    const data = layer === 'coal' ? COAL_COUNTRIES : PHOSPHATE_COUNTRIES;
    data.forEach(c => {
      ...
      const unit = layer === 'coal' ? 'Mt carbón' : 'Mt fosfatos';
      circle.bindPopup(`
        <div style="min-width:160px">
          <strong style="font-size:1rem">${c.name}</strong><br>
          <span style="font-size:.8rem;opacity:.7">Producción anual</span><br>
          <span style="font-size:1.1rem;font-weight:700;color:${c.color}">${c.produccion} ${unit}</span>
          ${c.reservas ? `<br><span style="font-size:.75rem;opacity:.6">Reservas: ${c.reservas.toLocaleString()} Mt</span>` : ''}
          ${c.tipo ? `<br><span style="font-size:.75rem">Tipo: ${c.tipo}</span>` : ''}
        </div>
      `);
      markers.push(circle);
    });
  ...
  return { initWorldMap, renderWorldMarkers, initColombiaMap, initTachiraMap, refreshTheme };
